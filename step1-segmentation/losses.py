...
def bhatta_tensor(tensor, bin = 5, eps = 10**-6):
    ...
    tensor = (tensor*bin).int()     # on passe les valeurs du tenseur de [0, 1] à [|0, bin|]
    H1 = torch.zeros([bin] + list(tensor.shape)[1:], device='cuda')     # l'histogramme
    for i in range(bin):
        bin_tensor = tensor==i
        bin_tensor = bin_tensor.sum(dim=0)
        H1[i] = bin_tensor          # le nombre d'occurence de la bin i trouvées dans les T estimations de chaque pixel 
    H1 = torch.sqrt(H1*(1/T))
    H2 = torch.flip(H1, [0])        # on retourne pour calculer Bhatta, et on a plus qu'à sommer
    # print('HIST :', H1.shape)
